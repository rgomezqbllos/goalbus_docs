...
def build_all(source_lang="ES", target_langs=None, report_dir=None):
    ...
    for p_folder in sorted(os.listdir(source_folder)):
        ...
        if not os.path.isdir(p_path) or not (p_folder.startswith("P") or p_folder.startswith("O") or p_folder.startswith("R")):
            continue
        for item in sorted(os.listdir(p_path)):
            source_path = os.path.join(p_path, item)
            if not os.path.isdir(source_path) or not (item.startswith("P") or item.startswith("O") or item.startswith("R")):
                continue
