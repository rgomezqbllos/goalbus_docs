...
1. En GoalBus, ve a  **Configuración** > **Personal** > **Reglas de turnos**.
...
4. Si no existe, crea un nuevo modelo pulsando en **Agregar Nuevo Modelo**.
...
Para gestionar las reglas de turnos de tu caso:
1. Si deseas crear una nueva regla, pulsa en **Añadir Nueva Regla**.
2. Dentro del modelo de reglas, revisa las **plantillas de reglas** disponibles y otorga un **Nombre** y una **Descripción** a la nueva regla.
3. Selecciona la plantilla que corresponda al control que quieres aplicar.
4. Crea una **regla específica** a partir de esa plantilla pulsando en **Confirmar**.
ref: P13_Imagen4.png | compact
6. Decide **a qué tipos de turnos se aplica cada regla**. No todas las reglas deben aplicarse a todos los tipos. Algunas pueden ser globales y otras deben dirigirse a categorías concretas, como mañana, tarde o partido.
7. Introduce los parámetros concretos de la regla.
8. Guarda la regla.
...
## Revisando que las reglas están asignadas al tipo de turno correcto

Una vez activadas las reglas, necesitas revisar **a qué tipos de turnos se aplica cada una**. No todas las reglas deben aplicarse a todos los tipos. Algunas pueden ser globales y otras deben dirigirse a categorías concretas, como mañana, tarde o partido.
...
Para revisar correctamente el ámbito de aplicación:
1. Selecciona cada regla que hayas creado.
2. Revisa la sección **Tipos de turnos aplicables**.
3. Selecciona los tipos concretos a los que debe aplicarse la regla.
4. Si la regla debe afectar a todos los tipos del escenario, configúrala como global seleccionando **todos los tipos de turno**.
